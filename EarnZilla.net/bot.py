#!/usr/bin/env python3
"""
EARNZILLA AUTO CLAIM + PTC TASKS + SHORTLINK HANDLING
- Handles FaucetPay Rewarded Ads automatically
- Faucet claims with shortlink solving (retries on failure)
...
def prompt_for_config():
    ...
    email = input("Earnzilla email: ").strip()
...
SYMBOL = "USDT"            # change to LTC, BTC, etc. as needed
...
HCAPTCHA_DOMAIN = "https://earnzilla.net/"

EARNZILLA_SITEKEY = "apv_f47e28152653294bf67360fc5f2802a57e48"
EARNZILLA_DOMAIN = "earnzilla.net"
...
def process_single_shortlink(access_token, provider):
    ...
    resp = requests.post("https://earnzilla.net/api/shortlinks/start",
    ...
    resp = requests.post("https://earnzilla.net/api/shortlinks/verify",
...
def try_all_shortlinks(access_token):
    ...
    resp = requests.get("https://earnzilla.net/api/shortlinks", headers=headers)
    resp.raise_for_status()
    providers = resp.json()
...
# ------------------- FAUCETPAY REWARDED AD HELPER -------------------
def skip_faucetpay_ad(access_token, ad_type):
    """Call the skip-ad-inventory endpoint to bypass the rewarded ad."""
    headers = {
        "Authorization": f"Bearer {access_token}",
        "User-Agent": USER_AGENT,
    ...
    resp = requests.post("https://earnzilla.net/api/faucets/skip-ad-inventory",
                         json=payload, headers=headers)
    resp.raise_for_status()
    data = resp.json()
    if data.get("success"):
        print("  ✅ FaucetPay ad skipped successfully.")
        return True
    else:
        print(f"  ❌ Skip ad failed: {data}")
        return False

# ------------------- PTC / BITCOTASK HELPERS (unchanged) -------------------
...
def process_ptc_tasks(session, access_token, user_id):
    ...
    resp = session.get("https://earnzilla.net/api/tasks/bitcotasks-ptc", headers=headers)
    resp.raise_for_status()
    data = resp.json()
...
# ------------------- EARNZILLA API HELPERS -------------------
def login(session, email, password, captcha_token, fp_hash, timezone):
    url = "https://earnzilla.net/api/auth/login"
    headers = {
        "User-Agent": USER_AGENT,
        "Content-Type": "application/json",
        "Origin": "https://earnzilla.net",
        "Referer": "https://earnzilla.net/",
    }
    payload = {
        "email": email,
...
def get_stats(session, token, symbol):
    url = "https://earnzilla.net/api/faucets/stats"
    headers = {"Authorization": f"Bearer {token}", "Referer": f"https://earnzilla.net/faucets/{symbol}", "User-Agent": USER_AGENT}
    resp = session.get(url, headers=headers)
    resp.raise_for_status()
    return resp.json()

def claim_faucet(session, token, symbol, captcha_token, user_id):
    url = "https://earnzilla.net/api/faucets/claim"
    ...
    headers = {
        ...
        "Content-Type": "application/json",
        "Origin": "https://earnzilla.net",
        "Referer": f"https://earnzilla.net/faucets/{symbol}",
...
# ------------------- FAUCET LOOP (with all ad/shortlink handling) -------------------
    ...
    while True:
        ...
        try:
            claim_token = solve_adslab_remote(
                sitekey=EARNZILLA_SITEKEY,
                domain=EARNZILLA_DOMAIN
            )
        except RuntimeError as e:
            ...
            if e.response.status_code == 429:
                ...
                if isinstance(error_body, dict) and "error" in error_body and "Shortlink" in error_body.get("error", ""):
                    ...
                    if try_all_shortlinks(access_token):
                        ...
                        break
                # Handle FaucetPay rewarded ad (or any ad with requiresAd)
                if isinstance(error_body, dict) and error_body.get("requiresAd"):
                    ad_type = error_body.get("adType")
                    print(f"  📺 FaucetPay ad required ({ad_type}) – skipping via API...")
                    if skip_faucetpay_ad(access_token, ad_type):
                        print("  ➡️ Ad skipped, retrying claim...")
                        time.sleep(1)
                        continue
                    else:
                        print("  ❌ Failed to skip ad, cannot continue faucet.")
                        break
                else:
        ...
        if resp.get("success"):
            ...
            if try_all_shortlinks(access_token):
                ...
                print("  ❌ Shortlink processing failed.")
                break
        else:
...
def main():
    ...
    print("🌐 Visiting earnzilla.net...")
    scraper.get("https://earnzilla.net/", timeout=30)
